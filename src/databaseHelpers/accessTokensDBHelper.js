...
function saveAccessToken(accessToken, userID, callback) {
  const getUserQuery = `INSERT INTO access_tokens (access_token, user_id) 
  VALUES ('${accessToken}', ${userID}) ON CONFLICT ON CONSTRAINT access_tokens_user_id_key DO UPDATE SET access_token = EXCLUDED.access_token`;
...
function getUserIDFromBearerToken(bearerToken, callback) {
  ...
  sqlConnection.query(getUserIDQuery, (dataResponseObject) => {
